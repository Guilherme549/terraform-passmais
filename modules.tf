...
module "passmais_ecs" {
  ...
  ARN_S3_env_backend                = var.ARN_S3_env_backend
  passmais_target_group_backend_arn = module.passmais_target_groups.target_group_backend_arn
}
...
module "passmais_rds" {
  ...
  db_name        = var.db_name
  banco_de_dados = local.banco_de_dados
  db_subnet_ids = [
    module.passmais_network.passmais_subnet_public_1a,
    module.passmais_network.passmais_subnet_public_1b
  ]
  db_instance_identifier_for_snapshot = var.db_instance_identifier_for_snapshot
}
...
module "passmais_loadbalancer" {
  ...
  target_group_backend_arn    = module.passmais_target_groups.target_group_backend_arn
  cert_validation_arn_backend = module.passmais_route53.acm_certificate_validated_backend_arn
